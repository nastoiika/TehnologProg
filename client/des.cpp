...
void Des::PermChoice1() //Выбор перестановки-1
{
    int k = 57, i;
    for (i = 0; i < 28; i++)
    {
...
void Des::Expansion() //Функция расширения, примененная на «правой» половине
{
    int exp[8][6], i, j, k;
    for (i = 0; i < 8; i++)
    {
...
char* Des::Decrypt(char* Text1, int key_des[64])
{
    for (int i = 0; i < 64; i++)
    {
        key[i] = key_des[i];
...
void Des::generate_key()
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        // Заполняем 7 бит случайными значениями и подсчитываем количество единиц
        for (int j = 0; j < 7; ++j)
        {
            key[i * 8 + j] = rand() % 2;
            if (key[i * 8 + j] == 1)
        ...
        // Восьмой бит (бит четности) устанавливаем так, чтобы общее количество единиц было нечетным
        if (ones_count % 2 == 0)
        {
            key[i * 8 + 7] = 1;
        }
